...
const deepsleepAdminApp = new Vue({
   el: '#deepsleepAdminApp',
    mounted(){
      ...
      if(this.getCookie('ds_auth')){
        this.ds_auth = JSON.parse(this.getCookie('ds_auth'));
    ...
    methods:{
      ...
      async prePopulateData(){
        ...
        const leaderboard = await this.fetchLeaderboard(this.getDateOffset());
        if(leaderboard){
      ...
      getDateOffset(){
        ...
        var today = new Date();
        var strToday =  today.getFullYear() + "-" + this.padTo2Digits(today.getMonth()+1) + "-" + this.padTo2Digits(today.getDate());
        // SleepNet's APIs all use 'date offsets' so I need to use this.
        var dateDelta = this.difference(this.datePickerDate, strToday);
      ...
      async mainProgram(dateOffset) {
        ...
        if (hypnoMeta) {
          ...
          this.cleanUpAllCharts();  // Get ready to draw new hypnos and biometrics by erasing old stuff
      ...
      async fetchLeaderboard(dateOffset){ 
        return await this.makeApiCall('https://sleepnet.appspot.com/api/admin/leaders/stat/window/Trued/sleep_efficiency/' + dateOffset + '/22/1/0/30', this.oldGetRequestOptions("GET"));
      },
      async fetchHealthkitData(dayOffset){
        return await this.makeApiCall('https://sleepnet.appspot.com/api/recordshour/' + dayOffset + "/22", this.getRequestOptions("GET"));
      },
      async fetchHypnoData(dayOffset){
        return await this.makeApiCall('https://sleepnet.appspot.com/api/hypnostats/' + dayOffset + "/22", this.getRequestOptions("GET"));
      },
      async fetchWhack2Data(model, dayOffset){
        return await this.makeApiCall('https://sleepnet.appspot.com/api/newwhack2/' + model + '/' + dayOffset, this.getRequestOptions("GET"));
      },
        ...
        .then(async response => {
           if(response.result != 'success'){
            ...
            if(this.login_params.remember_me){
              this.setCookie("ds_auth", JSON.stringify(response));
             }
             else{
              this.eraseCookie("ds_auth");
             }
      ...
      async logout(){
        const callback = ()=>{
          this.ds_auth = null;
          this.eraseCookie("_SleepNetSession");
          this.eraseCookie("ds_auth");
          location.reload();
        }
        fetch("https://sleepnet.appspot.com/api/logout", this.getRequestOptions('DELETE'))
        .then(result => {
          callback();
        })
        .catch(error => {
