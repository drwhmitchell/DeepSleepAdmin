...
function marshallSleepNetHypno(hypno) {
  ...
  newHypno.forEach((el, i) => {
    el.x = new Date(el.x).toLocaleString()
  ...
  if (sleepRec) {
    for (const rec of sleepRec) {
      if (rec.recordtype === hkType) {
        const newRec = {
          x: new Date(rec.startdate).toLocaleString(),
          y: rec.value
        };
...
function marshallHealthkitSleepRecords(sleepRec, startTime, endTime) {
  ...
  const newinBedRecs = [{ x: new Date(startTime).toLocaleString(), y: 5 }];

  fullInBedRecs.forEach(val => {
    newinBedRecs.push({ x: new Date(val.startdate).toLocaleString(), y: 4 });
    newinBedRecs.push({ x: new Date(val.enddate).toLocaleString(), y: 5 });
  });

  newinBedRecs.push({ x: new Date(endTime).toLocaleString(), y: 5 });
